  ...
  public initJQuery(currentMenuItem, subMenuItem) {
    ...
    $(document).ready(function () {
      ...
      $('.main-menu-item').mouseenter(function (e) {
        ...
        $('.main-menu-item .fa-caret-down').css('background-color', '$primary-color');
        $('.main-menu-item .fa-caret-down').css('border', 'none');
        $('.main-menu-item').css('background-color', '$primary-color');
        $('.main-menu-item').css('border-bottom', '3px solid var(--primary-color)');

        $(e.target).find('.fa-caret-down').css('background-color', '$secondary-color');
        $(e.target).find('.fa-caret-down').css('border', 'none');
